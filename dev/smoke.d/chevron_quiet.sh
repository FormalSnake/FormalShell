...
#          than the one it settles on, past Presence's own 0.97 zoom. A card
#          that opens around its cells starts at its padding alone and grows,
#          so a capture inside the entrance catches it.
...
# no quicker than a full frame did), while the entrance is `motion.surface`
# 180ms on OutQuint, which is 88% done in its first 60. The window in which a
# card opening around its cells is measurably narrow is therefore about one
# sample wide, and three openings catch it perhaps half the time: it caught
# 93px of 143px on one run of the code this leg was written against and 141px
# on the next. tests/tst_overflow_quiet.qml is where the gate itself is
# pinned. Do not tighten the sampling to chase it; the lanes below already
# lock in step under load.
#
...
leg_chevron_quiet_needs="convert"
...
# cover every cycle.
chevron_quiet_open_hold_ms=350
chevron_quiet_shut_hold=0.25
...
leg_chevron_quiet_timing() {
  # The drive below runs about thirteen seconds; the run's own frame is taken
...
leg_chevron_quiet_assert() {
  ...
  echo "chevron-quiet: settled card ${settled_width}px over $chevron_quiet_cycles openings"
  ...
  echo "chevron-quiet: narrowest card caught while opening ${narrowest}px of ${settled_width}px, over $opening frames"

  for i in "${!burst_index[@]}"; do
    if [ "${burst_width[$i]}" -eq 0 ] || [ "${burst_opening[$i]}" != yes ]; then continue; fi
    if [ $(( burst_width[i] * 100 )) -lt $(( settled_width * 90 )) ]; then
      fail "burst frame ${burst_index[$i]}, +${burst_offset[$i]}ms and inside an opening, caught the card ${burst_width[$i]}px wide against the ${settled_width}px it settled on: the card opened around its cells instead of with them ($chevron_quiet_burst_dir/burst-${burst_index[$i]}.png)"
    fi
  done
}
