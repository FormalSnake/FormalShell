...
import "../Radio/stations.js" as Stations

// Radio Atlas's player: the one mpv child, what it is playing, the saved
// favourites/recent/volume/output, and every Radio Browser request. The atlas
// (Surfaces/Radio/) drives it, and MediaService lists it as a now-playing
// source of its own rather than through MPRIS, so the media panel, the bar's
// now-playing cell and the `media` keybinds all reach it.
//
...
Singleton {
    ...
    // What mpv names its Pipewire stream (`--audio-client-name`), which is
    // how MediaService tells the radio's stream from any other mpv's: a
    // stream node carries no pid of its own, only its client does.
    readonly property string clientName: "FormalShell Radio"
    ...
    // cliamp's channels, fetched once per session: the list changes when
    // cliamp adds a channel, not while anyone is listening.
    property var cliampStations: []

    function fetchCliamp(done) {
        if (root.cliampStations.length > 0) {
            done(root.cliampStations);
            return;
        }
        root._run(["curl", "--fail", "--silent", "--connect-timeout", "4", "--max-time", "8",
            "--max-filesize", "65536", "--proto", "=https", "--user-agent", root.userAgent,
            Stations.CLIAMP_URL], (code, text) => {
            var rows = code === 0 ? Stations.parseCliamp(text) : null;
            if (rows && rows.length > 0)
                root.cliampStations = rows;
            done(rows && rows.length > 0 ? rows : null);
        });
    }
    ...
    function _mpvCommand() {
        var args = ["mpv", "--no-config", "--no-video", "--force-window=no", "--audio-display=no",
            ...
            "--audio-client-name=" + root.clientName];
        if (root.output !== "")
    ...
    // `auto` on a running player. The media panel's output menu names a sink
    // off Pipewire rather than off `outputs`, so a name is checked for shape
    // rather than against that list.
    function setOutput(id) {
        var sink = String(id || "");
        if (sink !== "" && !Stations.SINK_PATTERN.test(sink)) {
