...
// schema and cliamp's channel list. Pure, so RadioService.qml only
// orchestrates processes and files. The rules are radio-fetch's jq filter and radio-state's validator
...
var CLIAMP_URL = "https://radio.cliamp.stream/streams.m3u";
var CLIAMP_STREAM_PATTERN = /^https:\/\/radio\.cliamp\.stream\/([a-z0-9-]{1,64})\/stream$/;
var CLIAMP_EXCLUDED = ["omarchy"];
...
function parseCliamp(text) {
    ...
    for (var i = 1; i < lines.length; i++) {
        ...
        if (line.indexOf("#EXTINF:") === 0) {
            ...
            name = comma >= 0 ? _clean(line.slice(comma + 1).trim(), 160) : "";
            continue;
        }
        var match = CLIAMP_STREAM_PATTERN.exec(line);
        if (!match) {
        ...
        var slug = match[1];
        if (CLIAMP_EXCLUDED.indexOf(slug) < 0 && !out.some(function (s) { return s.uuid === "cliamp:" + slug; }))
            out.push(savedRecord({
                uuid: "cliamp:" + slug,
                name: name || slug,
                url: line,
