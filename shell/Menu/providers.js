...
// Name and keyword search over the vendored emoji dataset (emoji.json,
// generated by dev/gen-emoji.sh, parsed by Menu.qml). Six tiers: name exact
// > name starts-with > name word-start > keyword exact > name contains >
// keyword word-start, ties broken by file order, which puts base emoji ahead
// of their skin-tone variants for free. An empty query browses the whole
// set, so the summoned route is never an empty surface and never a truncated
// one either.
//
// The keyword tiers are CLDR's own annotations, the data macOS and iOS type
// their pickers against: Unicode's name for 😭 is "loudly crying face" and
// nothing else, so "sob" reaches it only through `kw`. They sit BELOW every
// name tier because a name match is the one the user can see on the row, and
// above `name contains` because an entry whose whole keyword is the query
// beats one that merely has the letters somewhere inside a longer word.
//
// `uses` (Core.State.emojiUses, frecency.js's record array) and `nowMs`
// reorder WITHIN each tier, never across: what gets used most leads the
// browse grid and leads among equally-good matches, and a better match still
// wins outright, the same contract appsProvider's frecency has.
//
...
function emojiSearch(list, query, uses, nowMs) {
    list = list || [];
    var q = String(query || "").trim().toLowerCase();
    if (!q) return _emojiRank(list.slice(), uses, nowMs);
    // Buckets rather than a comparator sort. There are exactly six tiers,
    // and the scan already visits the list in file order, so pushing into a
    // bucket and concatenating them highest-first produces the same order
    // (tier descending, file order within a tier) in one linear pass. A
    // one-letter query matches most of the 3944 entries, and sorting that
    // many with a comparator was the single biggest cost left on the route.
    var exact = [], prefix = [], wordStart = [], kwExact = [], substring = [], kwWord = [];
    for (var i = 0; i < list.length; i++) {
        var name = _emojiName(list[i]);
        var kw = _emojiKeywords(list[i]);
        if (name === q) exact.push(list[i]);
        else if (name.indexOf(q) === 0) prefix.push(list[i]);
        else if (name.indexOf(" " + q) >= 0) wordStart.push(list[i]);
        else if (kw.indexOf("|" + q + "|") >= 0) kwExact.push(list[i]);
        else if (name.indexOf(q) >= 0) substring.push(list[i]);
        else if (kw.indexOf("|" + q) >= 0 || kw.indexOf(" " + q) >= 0) kwWord.push(list[i]);
    }
    return _emojiRank(exact, uses, nowMs)
        .concat(_emojiRank(prefix, uses, nowMs),
                _emojiRank(wordStart, uses, nowMs),
                _emojiRank(kwExact, uses, nowMs),
                _emojiRank(substring, uses, nowMs),
                _emojiRank(kwWord, uses, nowMs));
}
...
// The entry's CLDR keywords wrapped in the pipe the generator joins them
// with, so a whole-keyword hit is one indexOf of "|q|" rather than a split
// per entry per keystroke. "" for an entry CLDR has nothing extra for, which
// matches no query, the `q` above being non-empty by then. Memoised for the
// same reason the name is; the file already stores them lowercased.
function _emojiKeywords(entry) {
    if (entry._kwIndex === undefined)
        entry._kwIndex = entry.kw ? "|" + String(entry.kw) + "|" : "";
    return entry._kwIndex;
}

// The ledger id for an emoji, the row id Menu.qml records on activation, so
// state.json stays readable ("emoji.😭") and both sides spell it once.
function emojiUsageId(entry) {
    return "emoji." + (entry ? entry.ch : "");
}

function _emojiUsageId(entry) {
    if (entry._usageId === undefined)
        entry._usageId = emojiUsageId(entry);
    return entry._usageId;
}
...
function emojiRows(list, query, paste, uses, nowMs) {
    var pasteAfter = paste !== false;
    var key = pasteAfter ? "_rowPaste" : "_rowCopy";
    return emojiSearch(list, query, uses, nowMs).map(function (e) {
        if (e[key] === undefined) {
            e[key] = {
                id: emojiUsageId(e),
                parentId: null,
