...
import qs.Services
import "../../Radio/model.js" as RadioModel
...
// every Radio Browser request live in Services/RadioService.qml, which is
// also what the media panel plays the radio through.
Item {
    id: root
    ...
    readonly property var displayStations: root.mode === "favorites"
        ? RadioService.favorites
        : (root.mode === "recent" ? RadioService.recent : root.results)
    ...
    readonly property var outputChoices: [{ id: "", label: "System default" }].concat(RadioService.outputs)
    readonly property string playingUuid: RadioService.station ? RadioService.station.uuid : ""
    ...
    function toggleOutputs() {
        ...
        if (!root.outputsVisible)
            return;
        var current = root.outputChoices.findIndex(o => o.id === RadioService.output);
        root.outputCursor = Math.max(0, current);
        RadioService.refreshOutputs();
    }

    function selectOutput(id) {
        RadioService.setOutput(id);
        root.outputsVisible = false;
    }
    ...
    function restorePlayingCountry(focusGlobe) {
        if (RadioService.running) {
            root.highlightStationCountry(RadioService.station, focusGlobe === true);
            return;
        }
        root.activeCountryCode = "";
        root.activeCountryName = "";
    ...
    function showCliamp() {
        ...
        root.restorePlayingCountry(false);
        RadioService.localError = "";
        root.setStationList("cliamp", RadioService.cliampStations);
        var token = root._beginFetch();
        RadioService.fetchCliamp(rows => {
            if (token !== root._fetchToken || root.mode !== "cliamp")
                return;
            root.fetching = false;
            if (!rows) {
                root.fetchError = "cliamp radio is unavailable. Try again shortly.";
                return;
            }
            root.setStationList("cliamp", rows);
        });
    }
    ...
    function search(text) {
        ...
        if (!root.previewSearch(query))
            return;
        var token = root._beginFetch();
        RadioService.search(query, rows => {
    ...
    function browseCountry(code, name) {
        ...
        var token = root._beginFetch();
        RadioService.fetchCountry(countryCode, root.worldStations, rows => {
    ...
    function playSelected() {
        ...
        if (root.remoteMode)
            RadioService.play(station, RadioModel.stationWindow(root.displayStations, station.uuid, 500));
        else
            RadioService.playFromSaved(station, root.displayStations);
    }
    ...
    function emptyStateText() {
        ...
        if (RadioService.localError)
            return RadioService.localError;
        ...
        if (root.mode === "cliamp")
            return "No cliamp channels found.";
    ...
    Connections {
        ...
        function onStationChanged() {
            var station = RadioService.station;
            if (!station)
                return;
            var index = RadioModel.indexByUuid(root.displayStations, station.uuid);
    ...
    FileView {
        path: Qt.resolvedUrl("../../Radio/countries.json").toString().replace(/^file:\/\//, "")
        watchChanges: false
        onLoaded: {
            try {
    ...
    Timer {
        ...
        onTriggered: {
            ...
            RadioService.fetchWorldMore(rows => {
    ...
    Item {
        ...
        Item {
            ...
            Globe {
                ...
                selectedStation: RadioService.station || root.selectedStation
            ...
            Text {
                ...
                anchors.bottom: parent.bottom
                readonly property string _error: root.fetchError || RadioService.localError
                text: mapHint._error !== ""
                    ? mapHint._error
                    : (root.activeCountryName
        ...
        Item {
            ...
            ButtonGroup {
                ...
                anchors.topMargin: Theme.space.panelPadding
                height: Theme.space.controlHeight
                readonly property var _modes: ["world", "cliamp", "favorites", "recent"]
                options: [
                    { label: "World", value: "world" },
                    { label: "cliamp", value: "cliamp" },
                    { label: "Favorites", value: "favorites" },
                    { label: "Recent", value: "recent" }
                ]
                index: tabs._modes.indexOf(root.mode)
                onPressed: i => {
                    if (i === 1)
                        root.showCliamp();
                    else if (i === 2)
            ...
            Item {
                ...
                ListView {
                    ...
                    delegate: Item {
                        ...
                        readonly property bool favorite: RadioService.isFavorite(stationRow.modelData.uuid)
                        ...
                        Cell {
                            ...
                            IconButton {
                                ...
                                onClicked: RadioService.toggleFavorite(stationRow.modelData)
                            }
                        }
                ...
                Text {
                    ...
                    textFormat: Text.PlainText
                    color: root.fetchError || RadioService.localError ? Theme.color.destructive : Theme.color.mutedForeground
                    font.family: Theme.fontFamilySans
                    font.pixelSize: Theme.fontSize.body
            ...
            Column {
                ...
                Repeater {
                    ...
                    delegate: Cell {
                        ...
                        ghost: true
                        interactive: true
                        selected: outputRow.modelData.id === RadioService.output
                ...
                Text {
                    leftPadding: Theme.space.controlPaddingX
                    visible: RadioService.outputs.length === 0 && !RadioService.outputsLoading
                    text: RadioService.outputsError || "No other audio outputs found"
                    textFormat: Text.PlainText
                    color: RadioService.outputsError ? Theme.color.destructive : Theme.color.mutedForeground
                    font.family: Theme.fontFamilySans
                    font.pixelSize: Theme.fontSize.caption
                }
            }
            ...
            Column {
                ...
                Item {
                    ...
                    Column {
                        ...
                        Text {
                            width: parent.width
                            text: RadioService.running
                                ? (String(RadioService.station.name || "").trim() || RadioService.title || "Unknown station")
                                : "Nothing playing"
                            textFormat: Text.PlainText
                            color: Theme.color.foreground
                            font.family: Theme.fontFamilySans
                            font.pixelSize: Theme.fontSize.body
                            font.weight: RadioService.running ? Theme.weight.semibold : Theme.weight.medium
                            elide: Text.ElideRight
                        }

                        Text {
                            readonly property bool _failed: RadioService.playerError !== "" || RadioService.error !== ""
                            width: parent.width
                            text: RadioService.playerError
                                ? RadioService.playerError
                                : RadioService.error
                                    ? RadioService.error + ". Play to retry, or Next."
                                    : !RadioService.running
                                        ? "Choose a signal to begin"
                                        : !RadioService.loaded
                                            ? "Connecting"
                                            : (RadioService.trackTitle ? RadioService.trackTitle + " · " : "")
                                                + (RadioService.paused ? "Paused" : "Live")
                                                + (RadioService.queue.length > 1 ? " · " + RadioService.queue.length + " stations queued" : "")
                            textFormat: Text.PlainText
                    ...
                    IconButton {
                        id: playingFavorite
                        readonly property bool favorite: RadioService.isFavorite(root.playingUuid)
                        anchors.right: parent.right
                        anchors.verticalCenter: parent.verticalCenter
                        visible: RadioService.running
                        name: "star"
                        ...
                        onClicked: RadioService.toggleFavorite(RadioService.station)
                    }
                }
                ...
                Item {
                    ...
                    ButtonGroup {
                        ...
                        exclusive: false
                        options: [
                            { icon: "skip-back", value: "previous", enabled: RadioService.running },
                            {
                                icon: RadioService.running && !RadioService.paused ? "pause" : "play",
                                value: "playpause",
                                enabled: RadioService.running || root.selectedStation !== null
                            },
                            { icon: "skip-forward", value: "next", enabled: RadioService.running },
                            { icon: "square", value: "stop", enabled: RadioService.running }
                        ]
                        onPressed: i => {
                    ...
                    Row {
                        ...
                        IconButton {
                            ...
                            tooltipText: RadioService.output
                                ? "Audio output: " + RadioService.outputLabel(RadioService.output)
                                : "Choose audio output"
                            onClicked: {
                                root.toggleOutputs();
                                root.forceActiveFocus();
                        ...
                        IconButton {
                            name: "volume-x"
                            variant: RadioService.muted ? "default" : "ghost"
                            enabled: RadioService.running
                            tooltipText: RadioService.muted ? "Unmute" : "Mute"
                            onClicked: RadioService.toggleMute()
                        }
                    }
                }
                ...
                Item {
                    ...
                    Icon {
                        ...
                        anchors.verticalCenter: parent.verticalCenter
                        name: RadioService.volume > 0 ? (RadioService.volume < 50 ? "volume-1" : "volume-2") : "volume-x"
                    ...
                    Track {
                        ...
                        MouseArea {
                            ...
                            function _setFromX(x) {
                                RadioService.setVolume(Math.max(0, Math.min(1, x / volumeTrack.width)) * 100);
                            }
