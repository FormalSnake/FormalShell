...
PanelWindow {
    ...
    readonly property real _contentHeight: header.height + root._headerGap
        + column.implicitHeight + drawer.padding * 2
    ...
    MouseArea {
        ...
        // Everything from the line to the card's own padding is the drawer's
        // (Components/Drawer.qml). The line it comes out from behind is the
        // frame ring's right side, or the bar's own hairline when the bar is
        // there; a bare screen with the bar elsewhere has neither, and the
        // card comes out from behind the output's own edge. The centre hangs
        // off the right of the output whatever edge the bar is on, since its
        // own x formula (geometry.js's centerFrame) puts it there.
        Drawer {
            id: drawer
            anchors.fill: parent
            owner: root
            ...
            edge: "right"
            screen: root._screen
            rect: Qt.rect(root._frame.x, root._frame.y, root.cardWidth, root._morphHeight)
            moving: morphHeight.running
            // The card rests a `screenPadding` off either end of its line and
            // has never run out to them (M57 D2).
            walls: false
                            ...
                            Item {
                                ...
                                Repeater {
                                    ...
                                    delegate: HistoryRow {
                                        ...
                                        // closing.
                                        readonly property bool _animated: drawer.presence.settled && root.isOpen
