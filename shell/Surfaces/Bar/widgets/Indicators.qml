...
// idiom whenever a real source shows up) and night light off
// NightLightService.active (M16 Task 6). The DND bell-off glyph this slot
...
// ttx, not memory: md-coffee U+F0176, md-lightbulb_night U+F1A4C. This
// `Row` is also what wakes NightLightService up at shell startup (a live
// binding on a QML singleton is what forces its lazy construction — see
// PolkitDialog.qml's own `PolkitService.flow` binding for the established
// precedent), so `nightlight.startOn` in settings.json actually takes
// effect even on a session where the indicator itself never renders.
Row {
    id: root
    ...
    // future reactivity.
    readonly property bool shown: root._idleInhibited || root._nightLightActive
