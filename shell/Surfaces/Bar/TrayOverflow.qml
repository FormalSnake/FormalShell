...
// The tray's second bar: the items the strip had no room for, in a card
// hanging off the toggle that owns them (widgets/Tray.qml). This is Ice's
// and Bartender's answer to a menu bar that has run out of edge, at the
// altitude the tray actually overflows at, rather than the bar's own chevron
...
Panel {
    ...
    // The first item this bar owns: everything from here on is what did not
    // fit inline. The owning Tray writes it on open and keeps it in step
    // while the fit moves under a bar that is still open.
    property int firstIndex: 0

    readonly property bool _vertical: Theme.barVertical
    readonly property int _count: Math.max(0, SystemTray.items.values.length - root.firstIndex)
    ...
    function _pointAt(index) {
        ...
        root.cursorIndex = index - root.firstIndex;
    }

    onCursorActivated: index => {
        var items = SystemTray.items.values;
        var item = items[root.firstIndex + index];
        if (!item)
            return;
    ...
    Rail {
        ...
        // The same live ObjectModel the strip renders, read from the other
        // end rather than from a `.values` slice: a plain-array model is a
        // full delegate reset on every one of Quickshell's re-notifies, and
        // every icon's async load restarts mid-decode with it
        // (widgets/Tray.qml's own Repeater carries the whole story).
        Repeater {
            ...
            delegate: TrayCell {
                ...
                visible: itemCell.index >= root.firstIndex
                cursor: root.cursorActive && itemCell.index - root.firstIndex === root.cursorIndex
