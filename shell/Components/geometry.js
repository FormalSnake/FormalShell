...
//
// `frameExtent` is the ANIMATED size, so a card that grows a second column
// stays centred on its cell for every frame of the morph rather than
// opening out of one side (owner, 2026-09-18: the media panel's lyrics pane
// left the card visibly off its cell). A cell close enough to either end
// hits the clamp first and keeps that edge on the padding while the rest of
// the growth goes the other way.
//
...
// pins the frame to its own inner edge instead, where a width change moves
// the leading edge on a right bar and nothing at all on a left one.
