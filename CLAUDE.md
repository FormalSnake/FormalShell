...
- `dev/smoke.sh` (`just vm-smoke <flags>`) is the rig now: nested Hyprland
  ...
  below still lives only in the niri script until M46 deletes it.
  `--hotcorner` reads `hyprctl -j layers` and asserts both corner surfaces
  sit on level 2, Hyprland's `top`. `--wallpaper`
