...
- `panel_emerge.sh` `--panel-emerge`: `panel open network` sampled frame by
  frame, the card growing out of the bar's line: a ladder of probes down the
  card's gutter showing its visible height grow from that line to the
  resting rect, and no card pixel inside the bar's own band in any frame.
