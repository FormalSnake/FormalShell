...
| Bar: chevron (collapse boundary) | VM-only | mac VM rig, 2026-08-14, `dev/smoke-niri.sh --chevron`, `docs/screenshots/chevron-collapsed-niri.png` / `chevron-expanded-niri.png` — `chevron` placed mid-right-region collapses the five entries after it; `bar chevron status` asserted in both states and the two screenshots differ by exactly those cells plus the glyph direction. The cell's own click is untested here (no synthetic pointer); `bar chevron expand` stands in for it, so a real pointer click is host-trial territory |
